...
export default function StreamList({
  ...
  return (
    <div className="space-y-4">
      {/* Demo Streams */}
      {Object.keys(demoStreams).length > 0 && (
        <div>
          ...
          <div className="space-y-2">
            {Object.entries(demoStreams).map(([name, url]) => (
              <button
                key={name}
                onClick={() => onLoadDemo(url)}
                className="w-full px-4 py-3 bg-slate-900/50 hover:bg-slate-800/70 border border-slate-700 hover:border-purple-500/50 rounded-xl text-left text-slate-200 transition-all duration-200 group"
              >
                <div className="flex items-center justify-between">
                  <span className="font-medium group-hover:text-purple-400 transition-colors">{name}</span>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <form onSubmit={onCustomUrl} className="space-y-3">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">HLS Stream URL or MP4</label>
                <input
                  type="text"
                  value={customUrl}
